function [ nlsmi, DF] = funObjNegLSMI_dis(W, X, Y, const, options, refInfo )
% 
...
else % CV should not be performed in this iteration
...
% sum1 = mean(Alpha'*(repmat(BKz, 1, n).*Ky(:, repmat(1:n, n, 1))))

if nargout > 1
    % Calculate the derivative
    DF = -deriveIByW(W, X, Xc, Alpha, Ky, BKz, best_sigmaz);
    
    % Use finite difference to check the calculation of DF
%     epsi = 1e-8;
%     eDF = zeros(m,1);
%     nlsmif = @(w)nlsmifunc(w,X,Xc, Ky, best_sigmaz, best_lsmilambda);
%     lw = nlsmif(W);
%     for k=1:m
%         E = zeros(m,1);
%         E(k) = epsi;
...
function nlsmi = nlsmifunc(W,X,Xc, Ky, sigmaz, lsmilambda)
    b = size(Xc,2);
    Z = bsxfun(@times, W, X);    
    Zc = bsxfun(@times, W, Xc);  
