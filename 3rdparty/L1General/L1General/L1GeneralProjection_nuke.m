function [w,fEvals,f] = L1GeneralProjection_nuke(gradFunc,w,lambda,params,refObj,varargin)
%
...
%   2: Second-order
%
% Sep 16, 2011: Modified to send a signal to the function before/after
% something is done. Allow one more stopping condition: If number of
% nonzero elements in the parameter vector is less than the specified, then
% stop.
%
...
infoRef.obj.G = {}; % cell array of gradient after each iteration
infoRef.obj.F = {}; % cell array of function value after each iteration
infoRef.obj.W = {}; % cell array of parameter vectors after each iteration
infoRef.obj.T = {}; % cell array of step size after each iteration

% Process input options
[verbose,maxIter,optTol,threshold,order,corrections,adjustStep, nzthreshold] = ...
    myProcessOptions(params,'verbose',1,'maxIter',250,...
    'optTol',1e-6,'threshold',1e-4,'order',2,'corrections',100,...
    'adjustStep',0, 'nzthreshold', 0);
...
% INIT_CALL signal
infoRef.obj.signal = OS.INIT_CALL;
varargin{end+1} = infoRef;
if order >= 2
    [f,g,H] = nonNegGrad(w,lambda,gradFunc,varargin{:});
else
    [f,g] = nonNegGrad(w,lambda,gradFunc,varargin{:});
end
infoRef.obj = rmfield(infoRef.obj, 'signal');
...
while fEvals < maxIter
    ...
    % Line Search
    if order >= 2
        [t,w,f,g,LSfunEvals,H] = ArmijoBacktrack_nuke(w,t,d,f,f,g,gtd,1e-4,2,optTol,...
            max(verbose-1,0),0,0,@projNonNegGrad,lambda,gradFunc,varargin{:});
    else
        [t,w,f,g,LSfunEvals] = ArmijoBacktrack_nuke(w,t,d,f,f,g,gtd,1e-4,2,optTol,...
    ...
    % Update log
    nzeroparams = sum(abs(w(1:p/2)-w(p/2+1:end))>threshold);
    if verbose
        fprintf('%10d %10d %15.5e %15.5e %15.5e %5d\n',i,fEvals,t,...
            f,sum(abs(g(free==1))), nzeroparams);
    end

    infoRef.obj.G{end+1} = g;
    infoRef.obj.F{end+1} = f;
    infoRef.obj.W{end+1} = w(1:length(w)/2)-w(length(w)/2 + 1:end);
    infoRef.obj.T{end+1} = t;
    ...
    % Stopping condition based on number of zero elements
    if nzeroparams <= nzthreshold
        break;
    end

    i = i +1;
    % Update iteration number in infoRef
    infoRef.obj.iteration = infoRef.obj.iteration + 1;
end
