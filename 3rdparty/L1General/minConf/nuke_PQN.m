function [x,f,funEvals,logs] = nuke_PQN(funObj,x,funProj,options, funFeedback)
...
% gradient algorithm
%
%   @funObj(x, signal): function to minimize (returns gradient as second argument)
%   @funProj(x): function that returns projection of x onto C
%	@funFeedback(logs): function to take logs struct arrays and return feedback signal
%
%   options:
...
%---------------------------------------------------------
% May 24, 2011
% Modified by Wittawat J. 
% - return logs keeping log of each iteration
% - Provide contextual signal to funObj to indicate the context in which it is
%   called. 
%
...
[f,g] = funObj(x, SIGNAL_INIT); % signal initial call with SIGNAL_INIT
funEvals = 1;
...
while funEvals <= maxIter
    ...
    [f_new,g_new] = funObj(x_new, SIGNAL_PRE_LINESEARCH ); % signal pre line search 
    funEvals = funEvals+1;
    ...
    while f_new > f + suffDec*g'*(x_new-x) || ~isLegal(f_new)
        ...
        x_new = x + t*d;
        %[f_new,g_new] = funObj(x_new);
        [f_new,g_new] = funObj(x_new, SIGNAL_LINESEARCH); % also signal linesearch
        funEvals = funEvals+1;
	...
	% p = size of the original variable (excluding the size of augmented variable)
	p = options.p; 
	...
	logs(i).funevals = funEvals*funEvalMultiplier; 
		
	% number of projections done so far
	logs(i).projevals = projects;
		
	% step length chosen in the line search
	logs(i).steplength = t;
		
	% optimality condition in the current iteration
	logs(i).optcond = optCond;
		
	% current x (optimization variable)
	logs(i).x = x(1:p);
    ...
    % Check optimality
    if ~isempty(funFeedback)
    	feedsig = funFeedback(logs);
    	if isequal(feedsig, FEEDBACK_STOP_OPT)
    		% funFeedback determines that the optimization should stop.
    		% See minConf_CONSTANTS.m
    		fprintf('funFeedback signals the stop of the optimization\n');
			break;    		
    	end
    end
