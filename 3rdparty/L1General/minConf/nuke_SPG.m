function [x,f,funEvals,projects,logs] = nuke_SPG(funObj,x,funProj,options)
% function [x,f] = nuke_SPG(funObj,x,funProj,options)
%
...
[verbose,numDiff,optTol,progTol,maxIter,suffDec,interp,memory,...
    useSpectral,curvilinear,feasibleInit,testOpt,bbType,testInit,optNorm] = ...
    myProcessOptions(...
    ...
    'testOpt',1,'bbType',1,'testInit',1,'optNorm',inf);
...
if numDiff
    ...
    % new definition of funcObj may not work here  ???
...
if testOpt && testInit
    projects = projects+1;
    if norm(funProj(x-g)-x,optNorm) < optTol
        if verbose >= 1
